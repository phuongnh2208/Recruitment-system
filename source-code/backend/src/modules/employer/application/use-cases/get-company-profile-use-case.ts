 ...
 *
 * Orchestrates the employer company profile retrieval flow following Clean Architecture
 ...
 * get company profile use case without containing business logic itself.
 *
 * ═════════════════════════════════════════════════════════════════
...
/**
 * Input DTO for the get company profile use‑case.
 */
export interface GetCompanyProfileCommand {
  /** The unique identifier of the user. */
  userId: string;
}

/**
 * Output DTO for the get company profile use‑case.
 */
export interface GetCompanyProfileResult {
  /** The employer profile entity. */
...
export class GetCompanyProfileUseCase {
  constructor(private readonly repository: IEmployerRepository) {}

  /**
   * Execute the get company profile flow.
   *
   * @param command - The get company profile command containing user ID.
   * @returns A GetCompanyProfileResult with the profile or null if not found.
   * @throws {ValidationException} If input validation fails.
   * @throws {InfrastructureException} If an unexpected error occurs.
   */
  async execute(command: GetCompanyProfileCommand): Promise<GetCompanyProfileResult> {
    try {
      ...
      logger.error(
        ...
        "Unexpected Error during company profile retrieval",
      );
      ...
      throw new InfrastructureException("Company profile retrieval failed", details);
    }
  }
}
