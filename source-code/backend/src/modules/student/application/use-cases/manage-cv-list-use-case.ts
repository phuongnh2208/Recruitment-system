 ...
 *   1. List CVs       -> validate input -> resolve StudentProfile.id -> repository.findByStudentId -> return list
 *   2. Delete CV      -> validate input -> resolve StudentProfile.id -> repository.findById -> verify owner ->
 *                       storage.delete -> repository.delete -> return success
 *   3. Set Default CV -> validate input -> resolve StudentProfile.id -> repository.findById -> verify owner ->
...
export interface ListCVCommand {
  /** The unique identifier of the student (User.id). */
  studentId: string;
}
...
export class ManageCVListUseCase {
  ...
  /**
   * Resolve StudentProfile.id from User.id.
  ...
  private async resolveStudentProfileId(userId: string): Promise<string> {
    const studentProfile = await this.studentProfileRepository.findByUserId(userId);
    if (!studentProfile) {
      logger.warn({ userId }, "Student Profile Not Found — no profile exists for the given userId");
      throw new NotFoundException("Student profile not found");
    }
    if (!studentProfile.id) {
      logger.warn({ userId }, "Student Profile ID is missing");
   ...
   *
   * @param command - The list CV command containing the student ID (User.id).
   * @returns A list of CV metadata entities.
   * @throws {ValidationException} If input validation fails.
   * @throws {NotFoundException} If student profile not found.
   */
  async list(command: ListCVCommand): Promise<ListCVResult> {
    try {
      ...
      // 2. Resolve StudentProfile.id from User.id
      const studentProfileId = await this.resolveStudentProfileId(command.studentId);

      // 3. Fetch CVs from repository using StudentProfile.id
      const cvs = await this.cvRepository.findByStudentId(studentProfileId);
   ...
   *
   * @param command - The delete CV command containing student ID (User.id) and CV ID.
   * @returns A success result.
   * @throws {ValidationException} If input validation fails.
   * @throws {NotFoundException} If the CV is not found or student profile not found.
  ...
  async delete(command: DeleteCVCommand): Promise<DeleteCVResult> {
    try {
      ...
      // 4. Verify ownership using StudentProfile.id
      if (cv.studentId !== studentProfileId) {
        logger.warn(
          {
            cvId: command.cvId,
            userId: command.studentId,
      ...
      // 6. Delete metadata from repository
   ...
   *
   * @param command - The set default CV command containing student ID (User.id) and CV ID.
   * @returns A success result.
   * @throws {ValidationException} If input validation fails.
   * @throws {NotFoundException} If the CV is not found or student profile not found.
   * @throws {AuthenticationException} If the student does not own the CV.
  ...
  async setDefault(command: SetDefaultCVCommand): Promise<SetDefaultCVResult> {
    try {
      ...
      // 4. Verify ownership using StudentProfile.id
      if (target.studentId !== studentProfileId) {
        logger.warn(
          {
            cvId: command.cvId,
            userId: command.studentId,
      ...
      // 5. Find current default CV using StudentProfile.id
      const currentDefault = await this.cvRepository.findDefaultByStudentId(studentProfileId);

      // 6. If a default CV exists and it's different from target, remove its default status
      if (currentDefault && currentDefault.id !== target.id) {
        currentDefault.removeDefault();
        await this.cvRepository.update(currentDefault);
      }

      // 7. Mark target as default
